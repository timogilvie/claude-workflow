...
if [[ ! -f "$MILL_SCRIPT" ]]; then
  ...
  if [[ -z "$HEREDOC_CONTENT" ]]; then
    ...
    CALLED_FUNCS=$(
      ...
      | grep -vE '^(try|catch|fromjson|add|rollout_path|thread_id|thread_row|updated_at|exits|setting|falling|select|strings|tostring)$' \
      | grep -vE '^(bad|budget|internal|keeping|marking|monitor|rate|reduce|relaunching|service|sparing|skipping|staying|timed|too|using|wavemill|waiting)$' \
