...
if [[ ! -f "$MILL_SCRIPT" ]]; then
  ...
  if [[ -z "$HEREDOC_CONTENT" ]]; then
    ...
    CALLED_FUNCS=$(
      ...
      | grep -vE '^(a|aborted|already|available|blocked_by_count|break|coding|cp|debug|empty_queue|execute|file|fresh|gtimeout|id|last_run|launch|length|main|mapfile|missing|not|now|overloaded|plan|ready|required|reservation|slots|the|they|timeout|todate|todateiso8601|tonumber|tracked|user)$')
