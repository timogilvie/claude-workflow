...
check_eq "merged PR disposition after release" "retained" "$(jq -r '.tasks["HOK-2600"].lifecycle.resourceDisposition' "$STATE_FILE")"
check_eq "merged PR retention reason reflects pane-released-git-retained" "pane-released-git-work-retained" "$(jq -r '.tasks["HOK-2600"].lifecycle.retention.reason' "$STATE_FILE")"
...
check_eq "paneReleased marker set true after release" "true" "$(jq -r '.tasks["HOK-2600"].terminalReconciliations["pr_merged:102"].paneReleased' "$STATE_FILE")"
...
check_eq "merged maps to release" "release" "$(wavemill_terminal_pane_policy pr_merged)"
check_eq "closed maps to release" "release" "$(wavemill_terminal_pane_policy pr_closed_unmerged)"
check_eq "challenge_resolved_winner maps to release" "release" "$(wavemill_terminal_pane_policy challenge_resolved_winner)"
check_eq "operator_abort maps to retain-manual" "retain-manual" "$(wavemill_terminal_pane_policy operator_abort)"
check_eq "recovery_failure maps to retain-manual" "retain-manual" "$(wavemill_terminal_pane_policy recovery_failure)"
check_eq "phase_launch_exhausted maps to retain-manual" "retain-manual" "$(wavemill_terminal_pane_policy phase_launch_exhausted)"
check_eq "review_complete maps to none" "none" "$(wavemill_terminal_pane_policy review_complete)"
check_eq "ready_complete maps to none" "none" "$(wavemill_terminal_pane_policy ready_complete)"

echo ""
echo "=== Phase Launch Exhausted Reason ==="

check_eq "phase_launch_exhausted is valid" "0" "$(wavemill_terminal_reason_valid phase_launch_exhausted; echo $?)"
check_eq "phase_launch_exhausted outcome is error" "error" "$(wavemill_terminal_workflow_outcome_for_reason phase_launch_exhausted)"
check_eq "phase_launch_exhausted hook state is error" "error" "$(wavemill_terminal_hook_state phase_launch_exhausted)"
check_eq "phase_launch_exhausted stage status is failed" "failed" "$(wavemill_terminal_stage_status_for_reason phase_launch_exhausted)"

echo ""
echo "=== Pane Release (tmux stub) ==="

reset_case "HOK-2700" "release-test" "200"
write_pr_state "200" "MERGED" "2026-08-01T12:00:00Z"
REPO_DIR="$TMP_DIR/repo-release"
mkdir -p "$REPO_DIR/.wavemill/evals/artifacts/HOK-2700"
wavemill_reconcile_terminal "$SESSION" "HOK-2700" "pr_merged" "200"
check_eq "merged PR paneReleased marked true" "true" "$(jq -r '.tasks["HOK-2700"].terminalReconciliations["pr_merged:200"].paneReleased' "$STATE_FILE")"
check_eq "tmux kill-window called for release" "true" "$(grep -q 'kill-window' "$TMUX_CALL_LOG" 2>/dev/null && echo true || echo false)"
REPO_DIR="$TMP_DIR"

reset_case "HOK-2701" "no-release-active" "201"
write_pr_state "201" "OPEN"
wavemill_reconcile_terminal "$SESSION" "HOK-2701" "review_complete" "201"
pane_released_val="$(jq -r '.tasks["HOK-2701"].terminalReconciliations["review_complete:201"].paneReleased | tostring' "$STATE_FILE")"
check_eq "review_complete does not release pane" "false" "$pane_released_val"

reset_case "HOK-2702" "no-release-abort" "202"
write_pr_state "202" "OPEN"
: > "$TMUX_CALL_LOG"
wavemill_reconcile_terminal "$SESSION" "HOK-2702" "operator_abort"
check_eq "operator_abort does not kill pane" "false" "$(grep -q 'kill-window' "$TMUX_CALL_LOG" 2>/dev/null && echo true || echo false)"
deferred_reason="$(jq -r '.tasks["HOK-2702"].terminalReconciliations["operator_abort"].paneReleaseDeferredReason // ""' "$STATE_FILE")"
check_eq "operator_abort records retain-manual deferral" "retain-manual" "$deferred_reason"

echo ""
echo "=== Pane Release Idempotency ==="

reset_case "HOK-2703" "idempotent-release" "203"
write_pr_state "203" "MERGED" "2026-08-01T12:00:00Z"
REPO_DIR="$TMP_DIR/repo-idem"
mkdir -p "$REPO_DIR/.wavemill/evals/artifacts/HOK-2703"
: > "$TMUX_CALL_LOG"
wavemill_reconcile_terminal "$SESSION" "HOK-2703" "pr_merged" "203"
kill_count1="$(grep -c 'kill-window' "$TMUX_CALL_LOG" 2>/dev/null || true)"
wavemill_reconcile_terminal "$SESSION" "HOK-2703" "pr_merged" "203"
kill_count2="$(grep -c 'kill-window' "$TMUX_CALL_LOG" 2>/dev/null || true)"
check_eq "second reconcile does not issue another kill-window" "$kill_count1" "$kill_count2"
REPO_DIR="$TMP_DIR"

echo ""
echo "=== Pane Release Gate Off ==="

reset_case "HOK-2704" "gate-off" "204"
write_pr_state "204" "MERGED" "2026-08-01T12:00:00Z"
REPO_DIR="$TMP_DIR/repo-gate"
mkdir -p "$REPO_DIR/.wavemill/evals/artifacts/HOK-2704"
: > "$TMUX_CALL_LOG"
WAVEMILL_TERMINAL_PANE_RELEASE=0 wavemill_reconcile_terminal "$SESSION" "HOK-2704" "pr_merged" "204"
check_eq "gate off still marks paneReleased" "true" "$(jq -r '.tasks["HOK-2704"].terminalReconciliations["pr_merged:204"].paneReleased' "$STATE_FILE")"
REPO_DIR="$TMP_DIR"
