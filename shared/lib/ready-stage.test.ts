...
describe('ready-stage', () => {
  ...
  describe('checkSchemaMigrations', () => {
    ...
    it('detects SQLAlchemy models under src/api/models/', () => {
      // Repro of the hokusai-data-pipeline gap where models live in
      // `src/api/models/<entity>.py` rather than a top-level `models.py`.
      const result = checkSchemaMigrations(
        ['src/api/models/benchmark_spec.py', 'src/app.ts'],
        '/tmp/test'
      );
      assert.equal(result.status, 'fail');
      assert.match(result.message, /without migration/);
    ...
    it('passes when SQLAlchemy model and alembic migration land together', () => {
      const result = checkSchemaMigrations(
        [
          'src/api/models/benchmark_spec.py',
          'migrations/versions/014_add_eval_spec.py',
        ],
        '/tmp/test'
      );
      assert.equal(result.status, 'pass');
    ...
    it('honors custom schemaPatterns from config', () => {
      const result = checkSchemaMigrations(
        ['db/queries.sqlx', 'src/app.ts'],
        '/tmp/test',
        { schemaPatterns: ['\\.sqlx$'] }
      );
      assert.equal(result.status, 'fail');
      assert.deepEqual(result.details?.schemaFiles, ['db/queries.sqlx']);
    });

    it('honors custom migrationPatterns from config', () => {
      const result = checkSchemaMigrations(
        ['app/models.py', 'db/changes/001.sql'],
        '/tmp/test',
        { migrationPatterns: ['db/changes/'] }
      );
      assert.equal(result.status, 'pass');
    });
  });

  describe('checkMigrationCiWiring', () => {
    let tempDir: string;

    beforeEach(async () => {
      tempDir = await fs.mkdtemp(path.join(os.tmpdir(), 'wavemill-migration-ci-'));
    });
    ...
    it('skips when no migration files in PR', () => {
      const result = checkMigrationCiWiring(['src/app.ts', 'README.md'], tempDir);
      assert.equal(result.status, 'skip');
      assert.equal(result.name, 'migration-ci-wiring');
    });

    it('fails when migration added but no runner found in CI config', async () => {
      const workflows = path.join(tempDir, '.github', 'workflows');
      ...
      const result = checkMigrationCiWiring(
        ['migrations/versions/014_add_col.py'],
        tempDir,
      );
      assert.equal(result.status, 'fail');
      assert.match(result.message, /no migration-runner invocation/);
      assert.deepEqual(result.details?.migrationFiles, ['migrations/versions/014_add_col.py']);
    });

    it('passes when alembic upgrade is present in a workflow', async () => {
      const workflows = path.join(tempDir, '.github', 'workflows');
      await fs.mkdir(workflows, { recursive: true });
      await fs.writeFile(
        path.join(workflows, 'deploy.yml'),
      ...
      assert.equal(result.status, 'pass');
      assert.match(result.message, /Migration runner found/);
    });

    it('passes when prisma migrate deploy appears in a script', async () => {
      const scripts = path.join(tempDir, 'scripts');
      await fs.mkdir(scripts, { recursive: true });
      await fs.writeFile(
        path.join(scripts, 'release.sh'),
    ...
    it('honors custom runner pattern from config', async () => {
      const workflows = path.join(tempDir, '.github', 'workflows');
      await fs.mkdir(workflows, { recursive: true });
      await fs.writeFile(
        path.join(workflows, 'deploy.yml'),
        'jobs:\n  step: run-custom-migrator --apply\n',
      );

      const result = checkMigrationCiWiring(
        ['migrations/2026_init.py'],
        tempDir,
        { runners: ['run-custom-migrator'] },
      );
      assert.equal(result.status, 'pass');
    });

    it('honors custom migrationPatterns from config', () => {
      // No config-aware migration files in the PR, so check should skip.
      const result = checkMigrationCiWiring(
        ['db/changes/001_add_col.sql'],
        tempDir,
        { migrationPatterns: ['db/changes/'] },
      );
      // No CI config in tempDir, but the new file IS a migration via config —
      // so this should fail (migration present, no runner).
      assert.equal(result.status, 'fail');
    });
  });
