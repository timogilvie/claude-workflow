...
describe('checkMetadata', () => {
  ...
  it('fails closed on unknown metadata keys', async () => {
    const result = await checkMetadata(buildContext({
      pr: {
    ...
    assert.equal(result.status, 'fail');
    assert.match(result.reason ?? '', /Unknown wavemill-meta field: future_key/);
    assert.deepEqual(result.labels, [WM_LABELS.metadataInvalid]);
  });

  it('reports dashed unknown metadata fields explicitly', async () => {
    const result = await checkMetadata(buildContext({
      pr: {
        body: [
          '<!-- wavemill-meta',
    ...
    assert.equal(result.status, 'fail');
    assert.match(result.reason ?? '', /review-infrastructure-note/);
  });
});
