...
/**
 * Extracts the current repository owner and name from git remote.
 *
 * @returns {{ owner: string, name: string }} Repository owner and name
 * @throws {Error} If git remote is not configured or URL is invalid
 */
export const getCurrentRepo = () => {
  try {
    ...
    throw new Error(`Failed to get current repository: ${error.message}`);
  }
};
