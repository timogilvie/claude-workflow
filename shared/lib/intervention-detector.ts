...
export function detectManualEdits(branchName: string, baseBranch: string, repoDir?: string): InterventionEvent {
  ...
  try {
    ...
    for (const record of records) {
      ...
      // Heuristic: commits without a known agent co-author tag are likely manual edits
...
export function detectAllInterventions(opts: DetectOptions): InterventionSummary {
  ...
  if (opts.prNumber) {
    ...
    postPrEvent = detectPostPrCommits(opts.prNumber, opts.repoDir);
  }
  ...
  if (branch) {
    manualEditEvent = detectManualEdits(branch, base, opts.repoDir);
