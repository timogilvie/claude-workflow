...
function routeWorkflowStageAwareInternal(
  ...
  const signalFloorPool = policyResolution?.policyStagePools
    ...
    : getModelPool(repoDir).models;
  const finalDecision = applySuspiciousZeroEscalation(
    attachSignalProvenance(withChallengeRecommendation(decision, repoDir), signalContext.classification),
...
export async function routeWorkflowHokusai(
  ...
  const hokusaiDecision = withChallengeRecommendation({
    ...enriched,
