...
async function main() {
  ...
  const backlog = await linearClient.issues({
    filter: {
      ...
      title: { contains: "Update user interface" }
    }
  });

  const parentIssue = backlog.nodes[0];
  if (!parentIssue) {
    throw new Error('Parent issue "Update user interface on Leads page" not found in backlog');
  }

  console.log(`Found parent issue: ${parentIssue.identifier} - ${parentIssue.title}`);

  // Get or create milestone for this epic
  // For now, skip milestone creation - it's optional
  const milestoneId = undefined;
  console.log(`Skipping milestone creation (optional feature)`);
