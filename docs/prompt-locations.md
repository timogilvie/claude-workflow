...
- `tools/prompts/native-read-only-phase.md`: **Native agent fork** — system prompt for native read-only planning. Loaded via `loadNativePhasePrompt()` in [shared/lib/native-agent/prompts.ts](/Users/timothyogilvie/Dropbox/wavemill/worktrees/native-runtime-wire-opt-in-native-planning-phase/shared/lib/native-agent/prompts.ts) and consumed by [shared/lib/native-agent/launch-planning.ts](/Users/timothyogilvie/Dropbox/wavemill/worktrees/native-runtime-wire-opt-in-native-planning-phase/shared/lib/native-agent/launch-planning.ts) and `shared/lib/native-agent/smoke.ts`. This is a fork from `tools/prompts/planning-phase.md` because the planning-phase template contains shell-specific placeholders (`{{TOOLS_DIR}}`, `{{ISSUE}}`, etc.) that do not apply to the TypeScript native agent path. Prompt usage is logged through `logPromptUsage()`, and the native adapter registers provider/model/api identity as a `runtime` resource plus the exposed phase tool set as an `agent-config` resource linked in the session manifest.
