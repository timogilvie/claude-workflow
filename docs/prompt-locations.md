...
- `tools/prompts/native-read-only-phase.md`: **Native agent fork** — system prompt for native read-only (planning) phases. Loaded via `loadNativePhasePrompt()` in `shared/lib/native-agent/smoke.ts` (the production native read-only launcher) using `logPromptUsage()` so the prompt registry row is written automatically. This is a fork from `tools/prompts/planning-phase.md` because the planning-phase template contains shell-specific placeholders (`{{TOOLS_DIR}}`, `{{ISSUE}}`, etc.) that do not apply to the TypeScript native agent path. The native agent adapter registers provider/model/api identity as a `runtime` resource and the phase-level tool set as an `agent-config` resource; both are linked to this prompt ref in the session manifest via `recordUse()`.
