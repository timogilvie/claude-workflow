...
- Each fresh planning-to-coding launch writes `features/<slug>/.coding-attempt.json`. Attempt-scoped gate artifacts from an earlier coding attempt are not reused: `.coding-complete` and `.coding-blocked-completion.json` are renamed with a `.stale.<utc-ts>` suffix, and derived auto-advance/attention markers are cleared with an audit in `features/<slug>/.coding-artifact-quarantine.json`.
